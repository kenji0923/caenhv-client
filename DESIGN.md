...
- Remote hosts: an opt-in TCP listener (`CAENHV_CLIENT_TCP_PORT`, with bind-address and shared-token options) accepts both the one-line show protocol and JSON control commands. Remote launch is impossible by design — the GUI is expected to auto-start at login on its host.
- Remote HV control: the GUI is the single gateway to devman, so programmatic control is done by sending JSON commands *to the GUI*, which executes them on its GUI thread through the same `ClientWorker` an operator edit uses — every remote setpoint passes through the link engine and all safeguards. Control commands are gated on a configured, matching token (`CAENHV_CLIENT_TCP_TOKEN`); with no token the channel is show/raise-only. Commands: `get`, `get_link` (linked/master_slot/master_channel/offset under `values`), `get_many` (bulk read of a channel list in one round-trip; per-channel `{vmon,vset,imon,power,status[,link]}`, `include_link` optional, one bad channel isolated as `{error}` at its index — a whole-channel failure. A channel whose core snapshot reads but whose `vset`/`power` sub-read fails is *partial*: it returns the readable keys, omits the failed one, and carries an `errors` map `{"vset"|"power": "<msg>"}` so a partial failure is visible rather than a silently missing key — consumers using `.get()` on the value keys are unaffected), `get_links`, `set_vset`, `set_offset`, `set_power` (linked groups applied whole, no dialog), `set_param` (rup/rdown/iset/trip/svmax/pdown), `set_linked_bulk` (apply many linked vset/offset changes atomically — seeds all requested changes before validation via `_build_linked_targets`, so a valid final state is not rejected at an intermediate single-channel step; link rules roll back on failure. When a rejection is attributable to one channel — a resulting Vset out of range or over SVMax, a conflicting linked target, or a linked-power mismatch — the error reply additionally carries a machine-addressable `channel` field, `"slot:ch"`; rejections not tied to a single channel keep the plain `{status,error}` shape. `RemoteClient` folds this into the `RuntimeError` message and exposes it as a `.channel` attribute). The `caenhv-client-python` package exposes these as functions and `RemoteClient` methods; safeguard rejections return as errors to the caller. The TCP listener keeps the connection open after each reply, so a client may hold one socket and pipeline many commands (`RemoteClient(persistent=True)`); one reply per command, in order.
